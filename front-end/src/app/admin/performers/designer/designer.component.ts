...
export class DesignerComponent implements OnInit, AfterContentInit {
  ...
  navigateToFormBilder({ element }) {
    ...
    this.base.get("node/byDiagramID/" + diagramId).subscribe((node) => {
      const { type } = element;
      if (type == "bpmn:Task") {
        this.router.navigate(["/admin/performers/task-performers"], {
  ...
  haveProcessDiagram() {
    this.dialogService
      .open(ErrorDialogComponent, {
        context: {
          title: "اخطار",
          discription: "نمودار فرایند طراحی نشده ابتدا نمودار را طراحی کنید",
        },
      })
      .onClose.subscribe(this.goBack.bind(this));
  }
  goBack() {
    this.router.navigate(["/admin/performers/list"]);
  }
}
